...
func getCdnFrontendBase() string {
	...
	if base == "" || base == "__OPENAGENT_FRONTEND_CDN_REPO_BASE__" {
		version := cli.Version
		if version == "" || version == "dev" {
			version = "__OPENAGENT_FRONTEND_VERSION__"
		}
		return fmt.Sprintf("__OPENAGENT_FRONTEND_CDN_REPO_BASE__@%s", version)
	}
	return base
}
...
func getCdnFrontendIndexHtml() (string, error) {
	...
	if len(scriptTags) == 0 || !slices.ContainsFunc(manifest.Entrypoints, func(entrypoint string) bool {
		return strings.HasSuffix(entrypoint, ".js")
	}) {
