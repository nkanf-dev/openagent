...
func browserUseChromeExtSnapshot(provider *BrowserUseTool, ctx context.Context) (string, error) {
	...
	return browserUseAugmentSnapshotWithWebSkills(browserUseWebSkillOwner(provider), snapshot.URL, formatted)
}
...
func chromeConnectBuiltinTools(provider *BrowserUseTool) []BuiltinTool {
	return []BuiltinTool{
		...
		&chromeConnectSaveWebSkillBuiltin{provider: provider},
		&chromeConnectListWebSkillsBuiltin{provider: provider},
		&chromeConnectDeleteWebSkillBuiltin{provider: provider},
...
func (b *chromeConnectTypeBuiltin) GetDescription() string {
	return browserUseWithWebSkillReflection("Type text into an indexed input-like element or a CSS selector from the latest browser_use_snapshot. Set clear=true to replace the current field content. Verify with browser_use_snapshot before relying on indexes or claiming the input was accepted.")
}
...
func (b *chromeConnectPressBuiltin) GetDescription() string {
	return browserUseWithWebSkillReflection("Press a keyboard key in the controlled Chrome tab, such as Enter, Tab, Escape, ArrowDown, or Space. A key press can submit a form, navigate, or change focus; call browser_use_snapshot before the next indexed action.")
}
...
func (b *chromeConnectPlayMediaBuiltin) GetDescription() string {
	return browserUseWithWebSkillReflection("Play and unmute visible audio or video elements on the Browser Use controlled tab via the OpenAgent Chrome extension. Use this after opening a page with media if playback is paused or muted.")
}
...
func (b *chromeConnectTabsBuiltin) GetDescription() string {
	return browserUseWithWebSkillReflection("List open Chrome tabs available via the OpenAgent Chrome extension, including active, controlled, and protected tab markers, titles, and URLs. Use this before switching tabs or when the current page does not match what the user sees.")
}
...
func (b *chromeConnectSwitchTabBuiltin) GetDescription() string {
	return browserUseWithWebSkillReflection("Switch Browser Use to a tab returned by browser_use_tabs via the OpenAgent Chrome extension. Protected OpenAgent UI tabs cannot be controlled. Returns a fresh snapshot and browser state for the selected tab.")
}
...
func (b *chromeConnectCloseTabBuiltin) GetDescription() string {
	return browserUseWithWebSkillReflection("Close a Chrome tab returned by browser_use_tabs via the OpenAgent Chrome extension. Use browser_use_tabs first, then pass the tab index to close.")
}
...
func (b *chromeConnectCloseBuiltin) GetDescription() string {
	return browserUseWithWebSkillReflection("Disconnect the OpenAgent Chrome extension bridge. Only use this when the user explicitly asks to stop browser use; do not use it between related follow-up tasks. Chrome tabs are left open.")
}
